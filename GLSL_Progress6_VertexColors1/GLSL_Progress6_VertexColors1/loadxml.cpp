...
{
	dObj.newVecArray = 0;
	dObj.verticesfound = 0, dObj.normalsfound = 0, dObj.uvmapfound = 0, dObj.colorsfound = 0, dObj.indicesfound = 0;
	dObj.verticescount = 0, dObj.normalscount = 0, dObj.uvmapscount = 0, dObj.colorscount = 0, dObj.indicescount = 0;
		...
		if(cur_node->type == XML_ELEMENT_NODE)
		{
			...
			while(attribute)
			{
				...
				*/
				...
				if(dObj.limitOne < (dObj.meshCount * 49) && strcmp((char*)attribute->children->content, meshAppendPositions) == 0)
				{
					...
					//_getch();
					dObj.limitOne+=1;
					printf("LIMIT: %d MESHC: %d\n", dObj.limitOne, (dObj.meshCount*49));
				}
				//if(strcmp((char*)attribute->children->content, "Cube-mesh-normals-array") == 0)
				if(dObj.limitOne < (dObj.meshCount * 49) && strcmp((char*)attribute->children->content, meshAppendNormals) == 0)
				{
					if(dObj.meshSkip <= dObj.limitOne)
					{
						dObj.normalsfound = 1;
				...
				if(dObj.limitOne < (dObj.meshCount * 49) && strcmp((char*)attribute->children->content, meshAppendMap0) == 0)
				{
					if(dObj.meshSkip <= dObj.limitOne)
					{
						dObj.uvmapfound = 1;
				...
				if(dObj.limitOne < (dObj.meshCount * 49) && strcmp((char*)attribute->children->content, meshAppendColors) == 0)
				{
					if(dObj.meshSkip <= dObj.limitOne)
					{
						dObj.colorsfound = 1;
