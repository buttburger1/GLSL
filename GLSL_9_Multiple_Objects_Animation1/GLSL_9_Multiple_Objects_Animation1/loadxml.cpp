...
void traverse_dom_trees(xmlNode*a_node)
{
	...
	for(cur_node = a_node; cur_node; cur_node = cur_node->next)
	{
		if(cur_node->type == XML_ELEMENT_NODE)
		{
			...
			if(strcmp((char*)cur_node->name, "ambient") == 0)
			{
				//printf("METHOD4\n");//_getch();
				//dObj.matrixCount++;
			}
				...
				if(dObj.meshTransform <= (dObj.meshIncrementCount + dObj.matrixCount))
				{
					...
					for(int i = 0; i <= slen2; i++)
					{
						...
						if((int)cur_node->content[i] == 32 || (char)cur_node->content[i] == '\0')
						{
							...
							dObj.storedmatrixtransform[dObj.vboNumber + dObj.matrixCount + dObj.currentMesh][loopIncrement2] = (float)atof((char*)stringarray);
