...
glm::vec3 tempRight, tempUp, tempForward;
glm::vec3 tempRight1, tempUp1, tempForward1;
glm::mat4 tempMatrix;

int loadCameraCount = 0;
int spotCount = 0;
		...
		for(int i = 0; i < dObj.meshCount; i++)
		{
			if(dObj.indicescount != 1)//if(dObj.indicescount != 0)
			{
				...
				if(texturefile != 0)
				{
					...
					dObj.Texture[(vboID+i)*4+0] = loadPngImage(texturefile,               imgwidth, imgheight, hasAlpha, &textureImage);
					if(dObj.Texture[(vboID+i)*4+0] == 0)
					{
						printf("STOP1\n");//_getch();
					}
					else if(dObj.Texture[(vboID+i)*4+0] != 0)
					{
						printf("STOP2\n");//_getch();
					}
					
					char tempNormal[255], tempSpecular[255];
					strcpy(tempNormal, texturefile);
					strcpy(tempSpecular, texturefile);
					//
					tempNormal[strlen(tempNormal)-4] = '\0';
					tempSpecular[strlen(tempSpecular)-4] = '\0';
					//
					char tempAppendNormal[255] = "_normal";
					char tempAppendSpecular[255] = "_specular";
					//
					strcat(tempNormal, tempAppendNormal);
					strcat(tempSpecular, tempAppendSpecular);
					//
					strcat(tempNormal, ".png");
					strcat(tempSpecular, ".png");
					printf("tempNormal: %s\n", tempNormal);
					

					//dObj.Texture[(vboID+i)*4+1] = loadPngImage("container2_normal.png", imgwidth, imgheight, hasAlpha, &textureImage);
					dObj.Texture[(vboID+i)*4+1] = loadPngImage(tempNormal, imgwidth, imgheight, hasAlpha, &textureImage);
					printf("tempSpecular: %s\n", tempSpecular);//_getch();
					//dObj.Texture[(vboID+i)*4+2] = loadPngImage("container2_specular.png", imgwidth, imgheight, hasAlpha, &textureImage);
					dObj.Texture[(vboID+i)*4+2] = loadPngImage(tempSpecular, imgwidth, imgheight, hasAlpha, &textureImage);

					if(dObj.Texture[(vboID+i)*4+1] == 0 && dObj.Texture[(vboID+i)*4+2] == 0)
					{
						printf("Got Texture\n");//_getch();
					}
					else
	...
	*/
	...
	if(dObj.notDrawArray[dObj.vboNumber] != vboID)
	{
				...
				//for(int ii = 0; ii < dObj.meshLoad[vboID]; ii++)
		{
			...
			//printf("vNum: %d\n", dObj.vboNumber);
			//printf("testSpotLight0: %d\n", dObj.loadLightSpotArray[0]);
			//printf("testSpotLight1: %d\n", dObj.loadLightSpotArray[1]);
			//printf("testSpotLight2: %d\n", dObj.loadLightSpotArray[2]);

			//printf("A: %d ii: %d vboN: %d\n", dObj.loadLightSpotArray[dObj.vboNumber], ii, dObj.vboNumber);
			//printf("vboN: %d ii: %d A: %d\n", dObj.vboNumber, ii, dObj.loadLightSpotArray[ii]);
			//printf("1: %d 2: %d\n", dObj.vboNumber, ii);


			//printf("vboN: %d ii: %d A: %d\n", dObj.vboNumber, ii, dObj.loadLightSpotArray[0]);
			//printf("vboN: %d ii: %d A: %d spotCount: %d\n", dObj.vboNumber, ii, dObj.loadLightSpotArray[spotCount], spotCount);

			if(dObj.vboNumber == dObj.loadCameraArray[loadCameraCount])
			{
				tempMatrix = dObj.TranslationMatrix;

				tempRight = glm::vec3(tempMatrix[0][0], tempMatrix[0][1], tempMatrix[0][2]);
				tempUp = glm::vec3(tempMatrix[1][0], tempMatrix[1][1], tempMatrix[1][2]);
				tempForward = glm::vec3(tempMatrix[2][0], tempMatrix[2][1], tempMatrix[2][2]);

				//printf("tempR: X: %f Y: %f Z: %f\n", tempRight.x, tempRight.y, tempRight.z);
				//printf("tempUp: X: %f Y: %f Z: %f\n", tempUp.x, tempUp.y, tempUp.z);
				//printf("tempForward: X: %f Y: %f Z: %f\n", tempForward.x, tempForward.y, tempForward.z);

				//printf("1X: %f Y: %f Z: %f W: %f\n", tempMatrix[0][0], tempMatrix[0][4], tempMatrix[0][8], tempMatrix[0][12]);
				//printf("2X: %f Y: %f Z: %f W: %f\n", tempMatrix[0][1], tempMatrix[0][5], tempMatrix[0][9], tempMatrix[0][13]);
				//printf("3X: %f Y: %f Z: %f W: %f\n", tempMatrix[0][2], tempMatrix[0][6], tempMatrix[0][10], tempMatrix[0][14]);
				//printf("4X: %f Y: %f Z: %f W: %f\n", tempMatrix[0][3], tempMatrix[0][7], tempMatrix[0][11], tempMatrix[0][15]);

				//printf("tempMatrix: %d cam: %d\n", dObj.loadCameraArray[loadCameraCount], loadCameraCount);//_getch();
				//loadCameraCount++;
			}
			
			//if(dObj.vboNumber == 5)
			//if(dObj.loadLightSpotArray[ii] == dObj.vboNumber)
			if(dObj.vboNumber == dObj.loadLightSpotArray[spotCount])
			{
				tempMatrix = dObj.TranslationMatrix;
				...
				tempUp = glm::vec3(tempMatrix[1][0], tempMatrix[1][1], tempMatrix[1][2]);
				tempForward = glm::vec3(tempMatrix[2][0], tempMatrix[2][1], tempMatrix[2][2]);
				//printf("%d - %d storedSpotLightArray: %f\n", spotCount, dObj.loadLightSpot, dObj.storedSpotLightArray[spotCount]);
				tempForward *= -(dObj.storedSpotLightArray[spotCount]);

				/*
				if(tempMatrix[0][8] > 0 && tempMatrix[0][9] < 0)tempForward *= -25;
				else if(tempMatrix[0][8] > 0 && tempMatrix[0][9] > 0)tempForward *= -25;
				else if(tempMatrix[0][8] < 0 && tempMatrix[0][9] < 0)tempForward *= -25;
				else if(tempMatrix[0][8] < 0 && tempMatrix[0][9] > 0)tempForward *= -25;
				else if(tempMatrix[0][9] == 0)tempForward *= -25;
				*/
				...
				//printf("tempMatrix: %d spot: %d\n", dObj.loadLightSpotArray[spotCount], spotCount);//_getch();
			...
			if(dObj.drawDisable[vboID+ii] == 0)
			{
					...
					if(vboOrderArray[dObj.vboNumber] == vboID || vboTRS > 0)
					{
						...
						if(testTime[dObj.vboNumber] < 1.0f && trsSwapArray[dObj.vboNumber] > 0 || vboTRS > 0)
						{
							...
							if(savedDoubleArray[dObj.vboNumber][playLoop[dObj.vboNumber]] == 0)//CUBIC SPLINE
							{
								...
								float nearestLocX = round(daeLocX * 100) / 100;
								float nearestLocY = round(daeLocY * 100) / 100;
								float nearestLocZ = round(daeLocZ * 100) / 100;
								//printf("1daeRot X: %f Y: %f Z: %f vboNumber: %d\n", nearestLocX, nearestLocY, nearestLocZ, dObj.vboNumber);
								...
								float nearestRotX = round(daeRotEulerX * 100) / 100;
								float nearestRotY = round(daeRotEulerY * 100) / 100;
								float nearestRotZ = round(daeRotEulerZ * 100) / 100;
								//printf("1daeRot X: %f Y: %f Z: %f vboNumber: %d\n", nearestRotX, nearestRotY, nearestRotZ, dObj.vboNumber);
								...
								float nearestScaleX = round(daeScaleX * 100) / 100;
								float nearestScaleY = round(daeScaleY * 100) / 100;
								float nearestScaleZ = round(daeScaleZ * 100) / 100;
								//printf("1daeScale X: %f Y: %f Z: %f vboNumber: %d\n", nearestScaleX, nearestScaleY, nearestScaleZ, dObj.vboNumber);
								//printf("1daeScale X: %f Y: %f Z: %f vboNumber: %d\n", tempVec3Scale.x, tempVec3Scale.y, tempVec3Scale.z, dObj.vboNumber);

								dObj.TranslationMatrix = glm::mat4(1.0f);
								dObj.TranslationMatrix = glm::translate(dObj.TranslationMatrix, (glm::vec3(nearestLocX, nearestLocY, nearestLocZ)));
								//dObj.TranslationMatrix = glm::translate(dObj.TranslationMatrix, (glm::vec3(daeLocX, daeLocY, daeLocZ)));
								...
								dObj.RotationMatrixArray[dObj.vboNumber] = glm::mat4(1.0f);
								dObj.RotationMatrixArray[dObj.vboNumber] = glm::rotate(dObj.RotationMatrixArray[dObj.vboNumber], nearestRotY * glm::pi<float>() / 180.0f, glm::vec3(0.0f, 1.0f, 0.0f));//dObj.RotationMatrixArray[dObj.vboNumber] = glm::rotate(dObj.RotationMatrixArray[dObj.vboNumber], daeRotEulerY * glm::pi<float>() / 180.0f, glm::vec3(0.0f, 1.0f, 0.0f));
								dObj.RotationMatrixArray[dObj.vboNumber] = glm::rotate(dObj.RotationMatrixArray[dObj.vboNumber], nearestRotZ * glm::pi<float>() / 180.0f, glm::vec3(0.0f, 0.0f, 1.0f));//dObj.RotationMatrixArray[dObj.vboNumber] = glm::rotate(dObj.RotationMatrixArray[dObj.vboNumber], daeRotEulerZ * glm::pi<float>() / 180.0f, glm::vec3(0.0f, 0.0f, 1.0f));
								dObj.RotationMatrixArray[dObj.vboNumber] = glm::rotate(dObj.RotationMatrixArray[dObj.vboNumber], nearestRotX * glm::pi<float>() / 180.0f, glm::vec3(1.0f, 0.0f, 0.0f));//dObj.RotationMatrixArray[dObj.vboNumber] = glm::rotate(dObj.RotationMatrixArray[dObj.vboNumber], daeRotEulerX * glm::pi<float>() / 180.0f, glm::vec3(1.0f, 0.0f, 0.0f));

								dObj.ScalingMatrix = glm::mat4(1.0f);
								dObj.ScalingMatrix = scale(mat4(), vec3(nearestScaleX, nearestScaleY, nearestScaleZ));//scalingX, scalingY, scalingZ));//1.0f, 1.0f, 1.0f))
								//dObj.ScalingMatrix = scale(mat4(), vec3(daeScaleX, daeScaleY, daeScaleZ));//scalingX, scalingY, scalingZ));//1.0f, 1.0f, 1.0f))
								...
								if(myCam == 2)
								{
									if(dObj.vboNumber == dObj.loadCameraArray[loadCameraCount])
									{
										glm::mat4 tM;
										//glm::vec3 tR, tU, tF, tRr, tRu, tRt;
										tM = dObj.TranslationMatrix;
										tR = glm::vec3(tM[0][0], tM[0][1], tM[0][2]);
										tU = glm::vec3(tM[1][0], tM[1][1], tM[1][2]);
										tF = glm::vec3(tM[2][0], tM[2][1], tM[2][2]);
										//printf("11X: %f Y: %f Z: %f\n", tR.x, tR.y, tR.z);
										//printf("22X: %f Y: %f Z: %f\n", tU.x, tU.y, tU.z);
										//printf("33X: %f Y: %f Z: %f\n", tF.x, tF.y, tF.z);

										tRr = glm::vec3(-dObj.RotationMatrixArray[dObj.vboNumber][0][0], -dObj.RotationMatrixArray[dObj.vboNumber][0][4], -dObj.RotationMatrixArray[dObj.vboNumber][0][8]);
										tRu = glm::vec3(dObj.RotationMatrixArray[dObj.vboNumber][0][1], dObj.RotationMatrixArray[dObj.vboNumber][0][5], dObj.RotationMatrixArray[dObj.vboNumber][0][9]);
										tRt = glm::vec3(-dObj.RotationMatrixArray[dObj.vboNumber][0][2], -dObj.RotationMatrixArray[dObj.vboNumber][0][6], -dObj.RotationMatrixArray[dObj.vboNumber][0][10]);
										//printf("a1X: %f Y: %f Z: %f W: %f\n", dObj.RotationMatrixArray[dObj.vboNumber][0][0], dObj.RotationMatrixArray[dObj.vboNumber][0][4], dObj.RotationMatrixArray[dObj.vboNumber][0][8], dObj.RotationMatrixArray[dObj.vboNumber][0][12]);
										//printf("a2X: %f Y: %f Z: %f W: %f\n", dObj.RotationMatrixArray[dObj.vboNumber][0][1], dObj.RotationMatrixArray[dObj.vboNumber][0][5], dObj.RotationMatrixArray[dObj.vboNumber][0][9], dObj.RotationMatrixArray[dObj.vboNumber][0][13]);
										//printf("a3X: %f Y: %f Z: %f W: %f\n", dObj.RotationMatrixArray[dObj.vboNumber][0][2], dObj.RotationMatrixArray[dObj.vboNumber][0][6], dObj.RotationMatrixArray[dObj.vboNumber][0][10], dObj.RotationMatrixArray[dObj.vboNumber][0][14]);
										//printf("a4X: %f Y: %f Z: %f W: %f\n", dObj.RotationMatrixArray[dObj.vboNumber][0][3], dObj.RotationMatrixArray[dObj.vboNumber][0][7], dObj.RotationMatrixArray[dObj.vboNumber][0][11], dObj.RotationMatrixArray[dObj.vboNumber][0][15]);

										//printf("11X: %f Y: %f Z: %f W: %f\n", dObj.TranslationMatrix[0][0], dObj.TranslationMatrix[0][4], dObj.TranslationMatrix[0][8], dObj.TranslationMatrix[0][12]);
										//printf("22X: %f Y: %f Z: %f W: %f\n", dObj.TranslationMatrix[0][1], dObj.TranslationMatrix[0][5], dObj.TranslationMatrix[0][9], dObj.TranslationMatrix[0][13]);
										//printf("33X: %f Y: %f Z: %f W: %f\n", dObj.TranslationMatrix[0][2], dObj.TranslationMatrix[0][6], dObj.TranslationMatrix[0][10], dObj.TranslationMatrix[0][14]);
										//printf("44X: %f Y: %f Z: %f W: %f\n", dObj.TranslationMatrix[0][3], dObj.TranslationMatrix[0][7], dObj.TranslationMatrix[0][11], dObj.TranslationMatrix[0][15]);

										//printf("A daeRot X: %f Y: %f Z: %f\n", daeRotEulerX, daeRotEulerY, daeRotEulerZ);

										//tR = glm::vec3(dObj.TranslationMatrix[0][0], dObj.TranslationMatrix[0][4], dObj.TranslationMatrix[0][8]);tR *= dObj.TranslationMatrix[0][12];
										//tU = glm::vec3(dObj.TranslationMatrix[0][1], dObj.TranslationMatrix[0][5], dObj.TranslationMatrix[0][9]);tU *= dObj.TranslationMatrix[0][13];
										//tF = glm::vec3(dObj.TranslationMatrix[0][2], dObj.TranslationMatrix[0][6], dObj.TranslationMatrix[0][10]);tF *= dObj.TranslationMatrix[0][14];
										//printf("11X: %f Y: %f Z: %f\n", tR.x, tR.y, tR.z);
										//printf("22X: %f Y: %f Z: %f\n", tU.x, tU.y, tU.z);
										//printf("33X: %f Y: %f Z: %f\n", tF.x, tF.y, tF.z);
										...
										*/
										...
										dObj.right = glm::vec3(tRr.x, tRr.y, tRr.z);
										dObj.up = glm::vec3(tRu.x, tRu.y, tRu.z);
										dObj.target = glm::vec3(tRt.x, tRt.y, tRt.z);
										//dObj.up = glm::cross(dObj.right, dObj.target);

										//eye = -(glm::vec3(daeLocX, daeLocY, daeLocZ));
										//eye = glm::vec3(daeLocX, daeLocY, daeLocZ);
										eye = glm::vec3(nearestLocX, nearestLocY, nearestLocZ);
						...
						else
						{
							...
							float nearestLocY = round(daeLocY * 100) / 100;
							float nearestLocZ = round(daeLocZ * 100) / 100;
							//printf("2daeLoc X: %f Y: %f Z: %f vboNumber: %d\n", nearestLocX, nearestLocY, nearestLocZ, dObj.vboNumber);
							...
							daeRotEulerY = tempVec3Rot.y;
							daeRotEulerZ = tempVec3Rot.z;
							//printf("daeRot X: %f Y: %f Z: %f vboNumber: %d\n", daeRotEulerX, daeRotEulerY, daeRotEulerZ, vboNumber);
							//daeRotEulerX = pVec3Rot[dObj.vboNumber][loopTest[dObj.vboNumber]].x;
							//daeRotEulerY = pVec3Rot[dObj.vboNumber][loopTest[dObj.vboNumber]].y;
							//daeRotEulerZ = pVec3Rot[dObj.vboNumber][loopTest[dObj.vboNumber]].z;
							//printf("daeRot X: %f Y: %f Z: %f vboNumber: %d\n", tempVec3Rot.x, tempVec3Rot.y, tempVec3Rot.z, dObj.vboNumber);
							float nearestRotX = round(daeRotEulerX * 100) / 100;
							float nearestRotY = round(daeRotEulerY * 100) / 100;
							float nearestRotZ = round(daeRotEulerZ * 100) / 100;
							//printf("2daeRot X: %f Y: %f Z: %f vboNumber: %d\n", nearestRotX, nearestRotY, nearestRotZ, dObj.vboNumber);
							...
							tempVec3Scale = vec3(pScale1[dObj.vboNumber][playLoop[dObj.vboNumber]], pScale3[dObj.vboNumber][playLoop[dObj.vboNumber]], pScale5[dObj.vboNumber][playLoop[dObj.vboNumber]]) * 
								...
								((testTime[dObj.vboNumber] * testTime[dObj.vboNumber]) * (3.0f - 2.0f * testTime[dObj.vboNumber])) +
								nextTangent * ((testTime[dObj.vboNumber] * testTime[dObj.vboNumber]) * (testTime[dObj.vboNumber] - 1.0f));
							//printf("2daeScale X: %f Y: %f Z: %f vboNumber: %d\n", tempVec3Scale.x, tempVec3Scale.y, tempVec3Scale.z, dObj.vboNumber);
							daeScaleX = tempVec3Scale.x;
							daeScaleY = tempVec3Scale.y;
							daeScaleZ = tempVec3Scale.z;
							float nearestScaleX = round(daeScaleX * 100) / 100;
							...
							if(myCam == 2)
							{
								if(dObj.vboNumber == dObj.loadCameraArray[loadCameraCount])
								{
									//printf("B daeRot X: %f Y: %f Z: %f\n", daeRotEulerX, daeRotEulerY, daeRotEulerZ);
									//printf("b1X: %f Y: %f Z: %f W: %f\n", dObj.RotationMatrixArray[dObj.vboNumber][0][0], dObj.RotationMatrixArray[dObj.vboNumber][0][4], dObj.RotationMatrixArray[dObj.vboNumber][0][8], dObj.RotationMatrixArray[dObj.vboNumber][0][12]);
									//printf("b2X: %f Y: %f Z: %f W: %f\n", dObj.RotationMatrixArray[dObj.vboNumber][0][1], dObj.RotationMatrixArray[dObj.vboNumber][0][5], dObj.RotationMatrixArray[dObj.vboNumber][0][9], dObj.RotationMatrixArray[dObj.vboNumber][0][13]);
									//printf("b3X: %f Y: %f Z: %f W: %f\n", dObj.RotationMatrixArray[dObj.vboNumber][0][2], dObj.RotationMatrixArray[dObj.vboNumber][0][6], dObj.RotationMatrixArray[dObj.vboNumber][0][10], dObj.RotationMatrixArray[dObj.vboNumber][0][14]);
									//printf("b4X: %f Y: %f Z: %f W: %f\n", dObj.RotationMatrixArray[dObj.vboNumber][0][3], dObj.RotationMatrixArray[dObj.vboNumber][0][7], dObj.RotationMatrixArray[dObj.vboNumber][0][11], dObj.RotationMatrixArray[dObj.vboNumber][0][15]);

									//printf("11X: %f Y: %f Z: %f\n", dObj.right.x, dObj.right.y, dObj.right.z);
									//printf("22X: %f Y: %f Z: %f\n", dObj.up.x, dObj.up.y, dObj.up.z);
									//printf("33X: %f Y: %f Z: %f\n", tempForward.x, tempForward.y, tempForward.z);
							...
							//printf("RESET vboNum: %d testTime: %f\n", dObj.vboNumber, testTime[dObj.vboNumber]);
						...
						//}
						//printf("vboNum: %d testTime: %f\n", dObj.vboNumber, testTime[dObj.vboNumber]);
						//printf("daeRot X: %f Y: %f Z: %f ii: %d playLoop: %d\n", daeRotEulerX, daeRotEulerY, daeRotEulerZ, ii, playLoop[ii]);
						//printf("daeRot X: %f Y: %f Z: %f i: %d playLoop: %d\n", daeRotEulerX, daeRotEulerY, daeRotEulerZ, i, playLoop[i]);
						testTime[dObj.vboNumber] += deltaTime * playbackSpeed;//testTime[dObj.vboNumber] += 0.1f;
					}
					else
					{
						//
				...
				*/
				...
				glUniform3f(glGetUniformLocation(myShader, "lightPos"), dObj.TranslationMatrix[0][12], dObj.TranslationMatrix[0][13], dObj.TranslationMatrix[0][14]);//modelX, modelY, modelZ);
				...
				for(int i = 0; i < dObj.loadLightPoint; i++)
				{
					if(dObj.vboNumber == dObj.loadLightPointArray[i])
					{
						...
						//glUniform3f(glGetUniformLocation(myShader, tempPointLightPosition), dObj.TranslationMatrix[0][12], dObj.TranslationMatrix[0][13], dObj.TranslationMatrix[0][14]);//glUniform3f(glGetUniformLocation(myShader, "pointLight.position"), 5.0f, 5.0f, 0.0f);//modelX, modelY, modelZ);
						glUniform3f(glGetUniformLocation(myShader, tempPointLightAmbient), 0.5f, 0.5f, 0.5f);//glUniform3f(glGetUniformLocation(myShader, "pointLight.ambient"), 0.5f, 0.5f, 0.5f);//1.0f, 1.0f, 1.0f);
						glUniform3f(glGetUniformLocation(myShader, tempPointLightDiffuse), 0.5f, 0.5f, 0.5f);//glUniform3f(glGetUniformLocation(myShader, "pointLight.diffuse"), 0.6f, 0.6f, 0.6f);//1.0f, 1.0f, 1.0f);
						glUniform3f(glGetUniformLocation(myShader, tempPointLightSpecular), 0.5f, 0.5f, 0.5f);//glUniform3f(glGetUniformLocation(myShader, "pointLight.specular"), 0.7f, 0.7f, 0.7f);//1.0f, 1.0f, 1.0f);
				...
				for(int i = 0; i < dObj.loadLightDirectional; i++)
				{
					...
					if(dObj.vboNumber == dObj.loadLightDirectionalArray[i])//if(dObj.vboNumber == 0)
					{
						...
						//{
							...
							glUniform3f(glGetUniformLocation(myShader, tempDirLightDiffuse), 1.0f, 1.0f, 1.0f);
							...
							glUniform3f(glGetUniformLocation(myShader, tempDirLightSpecular), 1.0f, 1.0f, 1.0f);
				...
				for(int i = 0; i <= dObj.loadLightSpot; i++)//for(int i = 0; i < dObj.loadLightSpot; i++)
				{
					...
					if(dObj.vboNumber == dObj.loadLightSpotArray[i])
					{
						...
						//glUniform3f(glGetUniformLocation(myShader, tempSpotLightPosition), 0.0f, 15.0f, 0.0f);

						//printf("temp X: %f Y: %f Z: %f\n", dObj.TranslationMatrix[0][12], dObj.TranslationMatrix[0][13], dObj.TranslationMatrix[0][14]);
						//glUniform3f(glGetUniformLocation(myShader, tempSpotLightDirection), dObj.target.x, dObj.target.y, dObj.target.z);

						//glUniform3f(glGetUniformLocation(myShader, tempSpotLightDirection), 0.0f, -15.0f, 0.0f);
						glUniform3f(glGetUniformLocation(myShader, tempSpotLightDirection), tempForward.x, tempForward.y, tempForward.z);
						//
						//tempForward.x, tempForward.y, tempForward.z);
						//
