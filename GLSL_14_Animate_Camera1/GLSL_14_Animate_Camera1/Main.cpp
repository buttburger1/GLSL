//BY BUTTBURGER 1/19/2024 EXPORT
#include "Common.h"
//////////////////////////////////////////////////////////////////////////////////////////////////////
GLFWwindow*window;
...
//INSIDE
float modelX = -5.0f, modelY = -5.0f, modelZ = -5.0f;
...
float eyeX = 0.0f, eyeY = 0.0f, eyeZ = 30.0f;//Initial position : on +Z
...
extern glm::vec3 tR, tU, tF, tRr, tRu, tRt;
...
int main()
{
	...
	animScaleTime = 0.0f;//animScaleTime = 1.0f;
	...
	for(;;)//for(float i = 0.0f; i < 1.0f; i += 0.1f)
	{
		...
		*/
		...
		if(myCam == 0)
		{
			...
			//printf("target X: %f Y: %f Z: %f vA: %f hA: %f\n", cos(verticalAngle) * sin(horizontalAngle), sin(verticalAngle), cos(verticalAngle) * cos(horizontalAngle), verticalAngle, horizontalAngle);
		...
		*/
		...
		//LOAD LIGHT
		//drawVBO(dObj.VBOloadedID, dObj.materialShaderID, "light.dae", 0, 0);
		...
		//FLOOR
		drawVBO(dObj.VBOloadedID, dObj.materialShaderID, "plane.dae", "wood.png", 0);
		...
		//LOAD 3D LIGHT SOURCE
		//for(int i = 1; i < dObj.loadLightPoint + 1; i++)
		//for(int i = 1; i < dObj.loadLightDirectional + 1; i++)
		//for(int i = 1; i < dObj.loadLightSpot + 1; i++)
		for(int i = 1; i < dObj.loadLightPoint + dObj.loadLightDirectional + dObj.loadLightSpot + 1; i++)
		{
			//printf("loadLightPoint: %d\n", i);
			//printf("loadLightPoint: %d\n", dObj.loadLightPointArray[i-1]);
			//drawVBO(dObj.VBOloadedID, dObj.lightShaderID, DAEsphere, 0, 1);
			drawVBO(dObj.VBOloadedID, dObj.lightShaderID, DAEsphere, 0, i);
		}
		...
		*/
		...
		{
			timer++;
			//printf("RESET FPS %f - %fms Time %f Last %f Delta %f\n", frames, 1000.0f/double(frames), currentTime, lastTime, deltaTime);
			//printf("FPS %f - %fms Time %f Last %f Delta %f animFPS: %f\n", frames, 1000.0f/double(frames), currentTime, lastTime, deltaTime, animSpeedCounter);
			frames = 0.01f;
			//animSpeedCounter = 0;
		}
		//https://www.glfw.org/docs/latest/group__input.html#gaa6cf4e7a77158a3b8fd00328b1720a4a
		//if(currentTime >= 15.0f)
		if(currentTime >= 86400.0f)//RESET TIME IN 24 HOURS
		{
			timer = 0;
			//
			frames = 0.01f;
