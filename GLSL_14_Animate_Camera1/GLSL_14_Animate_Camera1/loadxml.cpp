...
void matrixCheck(int lightType, char*str)
{
	for(int i = dObj.matrixIncrement; i <= dObj.matrixCount; i++)
	{
		//printf("meshVBO: %d meshVBOi: %d str: %s VBO: %d VBOi: %d\n", i, dObj.meshVBOi, str, dObj.meshVBO, dObj.meshVBOi);
		...
		else if(strcmp(str, storedMesh2DoubleArray[i]) != 0)
		{
			...
			{
				//if(dObj.meshVBOi >= dObj.matrixCount)
				//if(dObj.meshVBO + dObj.matrixCamCount <= dObj.matrixCount)
				//if(dObj.meshVBOi >= dObj.matrixCount - dObj.matrixLightCount)

				if(dObj.meshVBO < dObj.meshVBOi && dObj.meshVBOi <= dObj.meshVBO + 1)
				{
					dObj.loadCameraArray[dObj.loadLightSpot] = vboOrderCount;
					//dObj.loadLightSpot += 1;
					
					printf("CAMERA meshVBO: %d meshVBOi: %d vboNumber: %d vboOrderCount: %d\n", dObj.meshVBO, dObj.meshVBOi, dObj.vboNumber, vboOrderCount);_getch();
				}
				...
				if(dObj.meshVBO + dObj.matrixCamCount <= dObj.matrixCount)
				{
					if(lightType == 1)
					{
						dObj.loadLightPointArray[dObj.loadLightPoint] = vboOrderCount;
						dObj.loadLightPoint += 1;
						printf("loadLightPoint: %d VOC: %d meshVBOi: %d matrixCount: %d\n", dObj.loadLightPoint, vboOrderCount, dObj.meshVBOi, dObj.matrixCount);_getch();
					}
					if(lightType == 2)
					{
						dObj.loadLightDirectionalArray[dObj.loadLightDirectional] = vboOrderCount;
						dObj.loadLightDirectional += 1;
						printf("loadLightDirectional: %d VOC: %d meshVBOi: %d matrixCount: %d\n", dObj.loadLightDirectional, vboOrderCount, dObj.meshVBOi, dObj.matrixCount);_getch();
					}
					if(lightType == 3)
					{
						if(lightType == 3 && tempSpot == 2)
						{
							printf("SPOT3\n");
							tempSpot = 3;
						}
						dObj.loadLightSpotArray[dObj.loadLightSpot] = vboOrderCount;
						dObj.loadLightSpot += 1;
						printf("loadLightSpot: %d VOC: %d meshVBOi: %d matrixCount: %d\n", dObj.loadLightSpot, vboOrderCount, dObj.meshVBOi, dObj.matrixCount);_getch();
					}
				}
				printf("222\n");
...
void lightCheck(int lightType)
{
	printf("lightCounter1: %d\n", dObj.lightCounter);
	//printf("point lName: %s count: %d\n", storedLightName, dObj.camCounter + dObj.meshVBO);
	//printf("camCounter1: %d total: %d\n", dObj.camCounter, dObj.camCounter + dObj.lightSkip);
	for(int i = dObj.camCounter; i <= dObj.camCounter + dObj.meshVBO + dObj.lightSkip; i++)
	{
		printf("camCounter2: %d total: %d\n", dObj.camCounter, dObj.camCounter + dObj.lightSkip);
		if(strcmp(storedLightName, storedMesh2DoubleArray[i]) == 0)
		{
			printf("lCheck1: %d - %d - i: %d\n", dObj.meshVBOi, dObj.camCounter + dObj.meshVBO, i);
			printf("lightSkip1: %d\n", dObj.lightSkip);
			dObj.lightSkip++;
			//printf("lightSkip2: %d\n", dObj.lightSkip);
			//_getch();
		...
		else if(dObj.meshVBOi <= dObj.camCounter + dObj.meshVBO + dObj.lightSkip + 1)//else if(dObj.meshVBOi <= dObj.camCounter + dObj.meshVBO + dObj.lightSkip)
		{
			printf("lCheck2: %d - %d i: %d\n", dObj.meshVBOi, dObj.camCounter + dObj.meshVBO, i);
			//
			if(dObj.meshVBOi == 0)dObj.matrixCount++;
			printf("matrixCount: %d matrixIncrement: %d matrixCamCount: %d\n", dObj.matrixCount, dObj.matrixIncrement, dObj.matrixCamCount);
			printf("lCheck3: %d - %d\n", dObj.meshVBOi, dObj.camCounter + dObj.meshVBO);
			if(dObj.lightSkip == dObj.lightCounter)
			{
				strcpy(meshName, storedLightName);
				printf("storedLightName: %s lightSkip: %d\n", storedLightName, dObj.lightSkip);//_getch();
				matrixCheck(lightType, storedLightName);
			}
			//
			break;
		}
...
void traverse_dom_trees(xmlNode*a_node)
{
	...
	for(cur_node = a_node; cur_node; cur_node = cur_node->next)
	{
		if(cur_node->type == XML_ELEMENT_NODE)
		{
			...
			/*
			if(strcmp((char*)cur_node->name, "ambient") == 0)//CONFLICTS
			{
			...
			*/
			...
			while(attribute)
			{
				...
				if(strcmp((char*)cur_node->name, "node") == 0)
				{
					if(strcmp((char*)attribute->name, "id") == 0)
					{
						printf("storedMeshName: %s Name: %s\n", storedMeshName, (char*)attribute->children->content);//printf("storedMeshName: %s\n", storedMeshName);
						if(strcmp((char*)attribute->children->content, storedMeshName) == 0)
						{
							...
							{
								tempSpot = 0;
								dObj.storedSpotLightArray[dObj.loadLightSpot-1] = tempFloat;
								printf("%d SPOT4: %f\n", dObj.loadLightSpot-1, dObj.storedSpotLightArray[dObj.loadLightSpot-1]);
								_getch();
							}
						}
					}
				...
				if(strcmp((char*)cur_node->name, "camera") == 0)
				{
					...
					if(strcmp((char*)attribute->name, "id") == 0)
					{
						...
						{
							//printf("dObj.camSkip: %d\n", dObj.camSkip);
							if(strcmp(storedCamChar, storedMesh2DoubleArray[dObj.camSkip]) == 0)
							{
								printf("camSkip: %d\n", dObj.camSkip);
								dObj.camSkip++;
								//_getch();
						...
						if(dObj.camSkip == dObj.camCounter)
						{
							printf("matrixIncrement: %d matrixCamCount: %d camSkip: %d\n", dObj.matrixIncrement, dObj.matrixCamCount, dObj.camSkip);
							...
							if(dObj.matrixLock == 0)
							{
								...
								//if(dObj.matrixIncrement == 0 || dObj.matrixIncrement <= dObj.matrixCamCount + 1)
								if(dObj.matrixIncrement == 0 || dObj.matrixIncrement <= dObj.matrixCamCount + 1)
								{
									printf("matrixIncrement: %d matrixCamCount+1: %d\n", dObj.matrixIncrement, dObj.matrixCamCount + 1);
									//printf("storedMesh2DoubleArray: %s\n", storedMesh2DoubleArray[0]);
									//printf("storedMesh2DoubleArray: %s\n", storedMesh2DoubleArray[1]);
									//printf("storedMesh2DoubleArray: %s\n", storedMesh2DoubleArray[2]);
									//printf("storedMesh2DoubleArray: %s\n", storedMesh2DoubleArray[3]);
									matrixCheck(0, storedCamChar);//matrixCheck((char*)attribute->children->content);
								}
							}
						}

						//MAYBE PUT ARRAY HERE


						dObj.camCounter++;
						printf("camCounter2: %d\n", dObj.camCounter);
					}
					if(strcmp((char*)attribute->name, "name") == 0)
					{
						printf("testCAM: %d - %d\n", dObj.meshVBO, dObj.meshVBOi);
						//if(dObj.meshVBOi == dObj.meshVBO + 1)
						//if(dObj.meshVBOi == 0 || dObj.meshVBOi < dObj.matrixCamCount)
						if(dObj.meshVBOi == 0)
						{
							dObj.matrixCamCount++;
							printf("mCamCount: %d\n", dObj.matrixCamCount);
				...
				if(strcmp((char*)cur_node->name, "light") == 0)
				{
					...
					if(strcmp((char*)attribute->name, "id") == 0)
					{
						...
						storedLightName[strlen(storedLightName)-6] = '\0';
						strcpy(meshName, storedLightName);
						printf("storedLightName: %s\n", storedLightName);
					}
					if(strcmp((char*)attribute->name, "name") == 0)
					{
						//if(dObj.meshVBOi == 0 || dObj.meshVBOi == dObj.meshVBO + 1)dObj.matrixLightCount++;
						if(dObj.meshVBOi == 0)
						{
							dObj.matrixLightCount++;
						}
				...
				if(strcmp((char*)cur_node->name, "geometry") == 0 && dObj.meshVBOi == 0 || strcmp((char*)cur_node->name, "geometry") == 0 && dObj.meshVBOi <= dObj.meshVBO)
				{
					...
					if(dObj.matrixLock == 1 && dObj.meshVBOi <= dObj.meshVBO)
					{
						dObj.matrixLock = 0;
						dObj.matrixIncrement = 0;//dObj.matrixIncrement--;
						for(int i = 0; i < dObj.matrixCount; i++)
						{
							//printf("a: %d STR: %s\n", i, storedMesh2DoubleArray[i]);
							//printf("STR: %s matrixIncrement: %d\n", storedMesh2DoubleArray[dObj.matrixIncrement], dObj.matrixIncrement);
							//memset(storedMesh2DoubleArray[dObj.matrixIncrement], 0, sizeof(storedMesh2DoubleArray[dObj.matrixIncrement]));
							memset(storedMesh2DoubleArray[i], 0, sizeof(storedMesh2DoubleArray[i]));
							//printf("b: %d STR: %s\n", i, storedMesh2DoubleArray[i]);
						}
						//dObj.camSkip = 0;
						//dObj.lightSkip = 0;
					...
					if(strcmp((char*)attribute->name, "name") == 0)
					{
						...
						for(int i = 0; i <= dObj.meshVBOi; i++)
						{
							...
							{
								printf("Hello2 there: %s %d - %d\n", storedMesh2DoubleArray[i], i, dObj.meshVBOi);//printf("Hello2 there: %s %d - %d\n", storedMesh2DoubleArray[i], i, dObj.meshVBO);
							}
				...
				if(dObj.uvmapfound == 0 && dObj.colorsfound == 0)
				{
					dObj.noUVorColor = 1;
					//printf("noUVorColorVOC: %d\n", vboOrderCount);_getch();
				}
				...
				if(strcmp((char*)cur_node->name, "float_array") == 0)
				{
					if(strcmp((char*)attribute->name, "id") == 0)
					{
						...
						if(strcmp((char*)attribute->children->content, meshAppendlocationXinput) == 0 && dObj.xInputFound == 0){dObj.xInputFound = 1;printf("FOUND: %s\n", meshAppendlocationXinput);}//"Cube_location_X-input-array"//meshAppendlocationXinput//printf("FOUND: Cube_location_X-input-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendlocationXoutput) == 0 && dObj.xOutputFound == 0){dObj.xOutputFound = 1;printf("FOUND: %s\n", meshAppendlocationXoutput);}//"Cube_location_X-output-array"//meshAppendlocationXoutput//printf("FOUND: Cube_location_X-output-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendlocationXintangent) == 0 && dObj.xInTangentFound == 0){dObj.xInTangentFound = 1;printf("FOUND: %s\n", meshAppendlocationXintangent);}//"Cube_location_X-intangent-array"//meshAppendlocationXintangent//printf("FOUND: Cube_location_X-intangent-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendlocationXouttangent) == 0 && dObj.xOutTangentFound == 0){dObj.xOutTangentFound = 1;printf("FOUND: %s\n", meshAppendlocationXouttangent);}//"Cube_location_X-outtangent-array"//meshAppendlocationXouttangent//printf("FOUND: Cube_location_X-outtangent-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendlocationYinput) == 0 && dObj.yInputFound == 0){dObj.yInputFound = 1;printf("FOUND: %s\n", meshAppendlocationYinput);}//"Cube_location_Y-input-array"//meshAppendlocationYinput//printf("FOUND: Cube_location_Y-input-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendlocationYoutput) == 0 && dObj.yOutputFound == 0){dObj.yOutputFound = 1;printf("FOUND: %s\n", meshAppendlocationYoutput);}//"Cube_location_Y-output-array"//meshAppendlocationYoutput//printf("FOUND: Cube_location_Y-output-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendlocationYintangent) == 0 && dObj.yInTangentFound == 0){dObj.yInTangentFound = 1;printf("FOUND: %s\n", meshAppendlocationYintangent);}//"Cube_location_Y-intangent-array"//meshAppendlocationYintangent//printf("FOUND: Cube_location_Y-intangent-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendlocationYouttangent) == 0 && dObj.yOutTangentFound == 0){dObj.yOutTangentFound = 1;printf("FOUND: %s\n", meshAppendlocationYouttangent);}//"Cube_location_Y-outtangent-array"//meshAppendlocationYouttangent//printf("FOUND: Cube_location_Y-outtangent-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendlocationZinput) == 0 && dObj.zInputFound == 0){dObj.zInputFound = 1;printf("FOUND: %s\n", meshAppendlocationZinput);}//"Cube_location_Z-input-array"//meshAppendlocationZinput//printf("FOUND: Cube_location_Z-input-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendlocationZoutput) == 0 && dObj.zOutputFound == 0){dObj.zOutputFound = 1;printf("FOUND: %s\n", meshAppendlocationZoutput);}//"Cube_location_Z-output-array"//meshAppendlocationZoutput//printf("FOUND: Cube_location_Z-output-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendlocationZintangent) == 0 && dObj.zInTangentFound == 0){dObj.zInTangentFound = 1;printf("FOUND: %s\n", meshAppendlocationZintangent);}//"Cube_location_Z-intangent-array"//meshAppendlocationZintangent//printf("FOUND: Cube_location_Z-intangent-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendlocationZouttangent) == 0 && dObj.zOutTangentFound == 0){dObj.zOutTangentFound = 1;printf("FOUND: %s\n", meshAppendlocationZouttangent);}//"Cube_location_Z-outtangent-array"//meshAppendlocationZouttangent//printf("FOUND: Cube_location_Z-outtangent-array\n");
						////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
						if(strcmp((char*)attribute->children->content, meshAppendrotationeulerXinput) == 0 && dObj.xRotEulerInputFound == 0){dObj.xRotEulerInputFound = 1;printf("FOUND: %s\n", meshAppendrotationeulerXinput);}//"Cube_rotation_euler_X-input-array"//meshAppendrotationeulerXinput//printf("FOUND: Cube_rotation_euler_X-input-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendrotationeulerXoutput) == 0 && dObj.xRotEulerOutputFound == 0){dObj.xRotEulerOutputFound = 1;printf("FOUND: %s\n", meshAppendrotationeulerXoutput);}//"Cube_rotation_euler_X-output-array"//meshAppendrotationeulerXoutput//printf("FOUND: Cube_rotation_euler_X-output-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendrotationeulerXintangent) == 0 && dObj.xRotEulerInTangentFound == 0){dObj.xRotEulerInTangentFound = 1;printf("FOUND: %s\n", meshAppendrotationeulerXintangent);}//"Cube_rotation_euler_X-intangent-array"//meshAppendrotationeulerXintangent//printf("FOUND: Cube_rotation_euler_X-intangent-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendrotationeulerXouttangent) == 0 && dObj.xRotEulerOutTangentFound == 0){dObj.xRotEulerOutTangentFound = 1;printf("FOUND: %s\n", meshAppendrotationeulerXouttangent);}//"Cube_rotation_euler_X-outtangent-array"//meshAppendrotationeulerXouttangent//printf("FOUND: Cube_rotation_euler_X-outtangent-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendrotationeulerYinput) == 0 && dObj.yRotEulerInputFound == 0){dObj.yRotEulerInputFound = 1;printf("FOUND: %s\n", meshAppendrotationeulerYinput);}//"Cube_rotation_euler_Y-input-array"//meshAppendrotationeulerYinput//printf("FOUND: Cube_location_Y-input-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendrotationeulerYoutput) == 0 && dObj.yRotEulerOutputFound == 0){dObj.yRotEulerOutputFound = 1;printf("FOUND: %s\n", meshAppendrotationeulerYoutput);}//"Cube_rotation_euler_Y-output-array"//meshAppendrotationeulerYoutput//printf("FOUND: Cube_location_Y-output-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendrotationeulerYintangent) == 0 && dObj.yRotEulerInTangentFound == 0){dObj.yRotEulerInTangentFound = 1;printf("FOUND: %s\n", meshAppendrotationeulerYintangent);}//"Cube_rotation_euler_Y-intangent-array"//meshAppendrotationeulerYintangent//printf("FOUND: Cube_location_Y-intangent-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendrotationeulerYouttangent) == 0 && dObj.yRotEulerOutTangentFound == 0){dObj.yRotEulerOutTangentFound = 1;printf("FOUND: %s\n", meshAppendrotationeulerYouttangent);}//"Cube_rotation_euler_Y-outtangent-array"//meshAppendrotationeulerYouttangent//printf("FOUND: Cube_location_Y-outtangent-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendrotationeulerZinput) == 0 && dObj.zRotEulerInputFound == 0){dObj.zRotEulerInputFound = 1;printf("FOUND: %s\n", meshAppendrotationeulerZinput);}//"Cube_rotation_euler_Z-input-array"//meshAppendrotationeulerZinput//printf("FOUND: Cube_rotation_euler_Z-input-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendrotationeulerZoutput) == 0 && dObj.zRotEulerOutputFound == 0){dObj.zRotEulerOutputFound = 1;printf("FOUND: %s\n", meshAppendrotationeulerZoutput);}//"Cube_rotation_euler_Z-output-array"//meshAppendrotationeulerZoutput//printf("FOUND: Cube_rotation_euler_Z-output-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendrotationeulerZintangent) == 0 && dObj.zRotEulerInTangentFound == 0){dObj.zRotEulerInTangentFound = 1;printf("FOUND: %s\n", meshAppendrotationeulerZintangent);}//"Cube_rotation_euler_Z-intangent-array"//meshAppendrotationeulerZintangent//printf("FOUND: Cube_rotation_euler_Z-intangent-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendrotationeulerZouttangent) == 0 && dObj.zRotEulerOutTangentFound == 0){dObj.zRotEulerOutTangentFound = 1;printf("FOUND: %s\n", meshAppendrotationeulerZouttangent);}//"Cube_rotation_euler_Z-outtangent-array"//meshAppendrotationeulerZouttangent//printf("FOUND: Cube_rotation_euler_Z-outtangent-array\n");
						////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
						if(strcmp((char*)attribute->children->content, meshAppendscaleXinput) == 0 && dObj.xScaleInputFound == 0){dObj.xScaleInputFound = 1;printf("FOUND: %s\n", meshAppendscaleXinput);}//"Cube_scale_X-input-array"//meshAppendscaleXinput//printf("FOUND: Cube_scale_X-input-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendscaleXoutput) == 0 && dObj.xScaleOutputFound == 0){dObj.xScaleOutputFound = 1;printf("FOUND: %s\n", meshAppendscaleXoutput);}//"Cube_scale_X-output-array"//meshAppendscaleXoutput//printf("FOUND: Cube_scale_X-output-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendscaleXintangent) == 0 && dObj.xScaleInTangentFound == 0){dObj.xScaleInTangentFound = 1;printf("FOUND: %s\n", meshAppendscaleXintangent);}//"Cube_scale_X-intangent-array"//meshAppendscaleXintangent//printf("FOUND: Cube_scale_X-intangent-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendscaleXouttangent) == 0 && dObj.xScaleOutTangentFound == 0){dObj.xScaleOutTangentFound = 1;printf("FOUND: %s\n", meshAppendscaleXouttangent);}//"Cube_scale_X-outtangent-array"//meshAppendscaleXouttangent//printf("FOUND: Cube_scale_X-outtangent-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendscaleYinput) == 0 && dObj.yScaleInputFound == 0){dObj.yScaleInputFound = 1;printf("FOUND: %s\n", meshAppendscaleYinput);}//"Cube_scale_Y-input-array"//meshAppendscaleYinput//printf("FOUND: Cube_scale_Y-input-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendscaleYoutput) == 0 && dObj.yScaleOutputFound == 0){dObj.yScaleOutputFound = 1;printf("FOUND: %s\n", meshAppendscaleYoutput);}//"Cube_scale_Y-output-array"//meshAppendscaleYoutput//printf("FOUND: Cube_scale_Y-output-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendscaleYintangent) == 0 && dObj.yScaleInTangentFound == 0){dObj.yScaleInTangentFound = 1;printf("FOUND: %s\n", meshAppendscaleYintangent);}//"Cube_scale_Y-intangent-array"//meshAppendscaleYintangent//printf("FOUND: Cube_scale_Y-intangent-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendscaleYouttangent) == 0 && dObj.yScaleOutTangentFound == 0){dObj.yScaleOutTangentFound = 1;printf("FOUND: %s\n", meshAppendscaleYouttangent);}//"Cube_scale_Y-outtangent-array"//meshAppendscaleYouttangent//printf("FOUND: Cube_scale_Y-outtangent-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendscaleZinput) == 0 && dObj.zScaleInputFound == 0){dObj.zScaleInputFound = 1;printf("FOUND: %s\n", meshAppendscaleZinput);}//"Cube_scale_Z-input-array"//meshAppendscaleZinput//printf("FOUND: Cube_scale_Z-input-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendscaleZoutput) == 0 && dObj.zScaleOutputFound == 0){dObj.zScaleOutputFound = 1;printf("FOUND: %s\n", meshAppendscaleZoutput);}//"Cube_scale_Z-output-array"//meshAppendscaleZoutput//printf("FOUND: Cube_scale_Z-output-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendscaleZintangent) == 0 && dObj.zScaleInTangentFound == 0){dObj.zScaleInTangentFound = 1;printf("FOUND: %s\n", meshAppendscaleZintangent);}//"Cube_scale_Z-intangent-array"//meshAppendscaleZintangent//printf("FOUND: Cube_scale_Z-intangent-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendscaleZouttangent) == 0 && dObj.zScaleOutTangentFound == 0){dObj.zScaleOutTangentFound = 1;printf("FOUND: %s\n", meshAppendscaleZouttangent);}//"Cube_scale_Z-outtangent-array"//meshAppendscaleZouttangent//printf("FOUND: Cube_scale_Z-outtangent-array\n");
					}
				}
				if(strcmp((char*)cur_node->name, "name_array") == 0)
				{
					if(strcmp((char*)attribute->name, "id") == 0)
					{
						if(strcmp((char*)attribute->children->content, meshAppendlocationXinterpolation) == 0 && dObj.xInterpolationFound == 0){dObj.xInterpolationFound = 1;printf("FOUND: %s\n", meshAppendlocationXinterpolation);}//"Cube_location_X-interpolation-array"//printf("FOUND: Cube_location_X-interpolation-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendlocationYinterpolation) == 0 && dObj.yInterpolationFound == 0){dObj.yInterpolationFound = 1;printf("FOUND: %s\n", meshAppendlocationYinterpolation);}//"Cube_location_Y-interpolation-array"//printf("FOUND: Cube_location_Y-interpolation-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendlocationZinterpolation) == 0 && dObj.zInterpolationFound == 0){dObj.zInterpolationFound = 1;printf("FOUND: %s\n", meshAppendlocationZinterpolation);}//"Cube_location_Z-interpolation-array"//printf("FOUND: Cube_location_Z-interpolation-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendrotationeulerXinterpolation) == 0 && dObj.xRotEulerInterpolationFound == 0){dObj.xRotEulerInterpolationFound = 1;printf("FOUND: %s\n", meshAppendrotationeulerXinterpolation);}//"Cube_rotation_euler_X-interpolation-array"//printf("FOUND: Cube_rotation_euler_X-interpolation-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendrotationeulerYinterpolation) == 0 && dObj.yRotEulerInterpolationFound == 0){dObj.yRotEulerInterpolationFound = 1;printf("FOUND: %s\n", meshAppendrotationeulerYinterpolation);}//"Cube_rotation_euler_Y-interpolation-array"//printf("FOUND: Cube_rotation_euler_Y-interpolation-array\n");
						if(strcmp((char*)attribute->children->content, meshAppendrotationeulerZinterpolation) == 0 && dObj.zRotEulerInterpolationFound == 0){dObj.zRotEulerInterpolationFound = 1;printf("FOUND: %s\n", meshAppendrotationeulerZinterpolation);}//"Cube_rotation_euler_Z-interpolation-array"//printf("FOUND: Cube_rotation_euler_Z-interpolation-array\n");
					}
					if(strcmp((char*)attribute->children->content, meshAppendscaleXinterpolation) == 0 && dObj.xScaleInterpolationFound == 0){dObj.xScaleInterpolationFound = 1;printf("FOUND: %s\n", meshAppendscaleXinterpolation);}//"Cube_scale_X-interpolation-array"//printf("FOUND: Cube_scale_X-interpolation-array\n");
					if(strcmp((char*)attribute->children->content, meshAppendscaleYinterpolation) == 0 && dObj.yScaleInterpolationFound == 0){dObj.yScaleInterpolationFound = 1;printf("FOUND: %s\n", meshAppendscaleYinterpolation);}//"Cube_scale_Y-interpolation-array"//printf("FOUND: Cube_scale_Y-interpolation-array\n");
					if(strcmp((char*)attribute->children->content, meshAppendscaleZinterpolation) == 0 && dObj.zScaleInterpolationFound == 0){dObj.zScaleInterpolationFound = 1;printf("FOUND: %s\n", meshAppendscaleZinterpolation);}//"Cube_scale_Z-interpolation-array"//printf("FOUND: Cube_scale_Z-interpolation-array\n");
				}
				
				if(strcmp((char*)cur_node->name, "dist") == 0 && tempSpot != 3)
				{
					//printf("FOUND: dist\n");
					if(strcmp((char*)attribute->children->content, "blender_dist") == 0)
					{
						printf("SPOT1: blender_dist\n");
						tempSpot = 1;
					}
					//_getch();
				}
		...
		if(cur_node->type == XML_TEXT_NODE)
		{
			...
			if(dObj.matrixtransformfound)// && dObj.meshTransform == dObj.currentMesh)
			{
				...
				if(dObj.meshTransform <= (dObj.meshIncrementCount + dObj.matrixCount))
				{
					...
					printf("LIST: %s\nMesh: %d - %d vboNum: %d total: %d\n", 
						(char*)cur_node->content, dObj.meshTransform, dObj.meshVBOi, dObj.vboNumber, dObj.vboNumber + dObj.meshVBOi);
					//
					dObj.matrixAll += 1;
					//
					for(int i = 0; i <= slen2; i++)
					{
						...
						if((int)cur_node->content[i] == 32 || (char)cur_node->content[i] == '\0')
						{
							...
							dObj.storedmatrixtransform[dObj.vboNumber + dObj.meshVBOi][loopIncrement2] = (float)atof((char*)stringarray);
							//
			...
			if(tempSpot == 1)
			{
				...
				tempFloat = (float)atof((char*)cur_node->content);
				//printf("tempFloat: %f\n", tempFloat);
				tempSpot = 2;
				//_getch();
			}
			...
			*/
			...
			if(dObj.newVecArray > 0 || dObj.indicesfound == 1)
			{
				...
				//KEEP BELOW 15000 TRIANGLES
				std::string testarray;
				testarray.reserve(1000000);
				//testarray.reserve(10000000);
				//std::cout<<testarray.max_size()<<std::endl;_getch();
