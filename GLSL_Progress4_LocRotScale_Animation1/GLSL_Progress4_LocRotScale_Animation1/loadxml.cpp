		...
		if(cur_node->type == XML_ELEMENT_NODE)
		{
			...
			while(attribute)
			{
				...
				*/
				...
				if(dObj.limitOne < (dObj.meshCount * 39) && strcmp((char*)attribute->children->content, meshAppendPositions) == 0)
				{
					...
					//_getch();
					dObj.limitOne+=1;
					printf("LIMIT: %d MESHC: %d\n", dObj.limitOne, (dObj.meshCount*39));
				}
				//if(strcmp((char*)attribute->children->content, "Cube-mesh-normals-array") == 0)
				if(dObj.limitOne < (dObj.meshCount * 39) && strcmp((char*)attribute->children->content, meshAppendNormals) == 0)
				{
					if(dObj.meshSkip <= dObj.limitOne)
					{
						dObj.normalsfound = 1;
				...
				if(dObj.limitOne < (dObj.meshCount * 39) && strcmp((char*)attribute->children->content, meshAppendMap0) == 0)
				{
					if(dObj.meshSkip <= dObj.limitOne)
					{
						dObj.uvmapfound = 1;
