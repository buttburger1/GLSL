	...
	*/
	...
	if(dObj.loadOnce[vboID] == 0)
	{
		...
		for(int i = 0; i < meshCount; i++)
		{
			...
			Texture[(vboID+i)*3+0] = loadPngImage("container2.png",          imgwidth, imgheight, hasAlpha, &textureImage);
			Texture[(vboID+i)*3+1] = loadPngImage("container2_specular.png", imgwidth, imgheight, hasAlpha, &textureImage);
			Texture[(vboID+i)*3+2] = loadPngImage("matrix.png",              imgwidth, imgheight, hasAlpha, &textureImage);
			...
			diffuseID[vboID+i] = glGetUniformLocation(myShader, "material.diffuse");//DIFFUSE(TEXTURE) MAP
			specularID[vboID+i] = glGetUniformLocation(myShader, "material.specular");//SPECULAR MAP
			emissionID[vboID+i] = glGetUniformLocation(myShader, "material.emission");//EMISSION MAP


			glGenVertexArrays(1, &VertexArrayID[vboID+i]);
			glGenBuffers(1, &vertexbuffer[vboID+i]);
			glGenBuffers(1, &uvbuffer[vboID+i]);
			glGenBuffers(1, &normalbuffer[vboID+i]);
			glGenBuffers(1, &elementbuffer[vboID+i]);


			glBindVertexArray(VertexArrayID[vboID+i]);
			//VERTEXBUFFER
			glBindBuffer(GL_ARRAY_BUFFER, vertexbuffer[vboID+i]);
			//glBufferData(GL_ARRAY_BUFFER, vectorvertices[vboLoop].size() * sizeof(glm::vec3), &vectorvertices[vboLoop][0], GL_STATIC_DRAW);
			glBufferData(GL_ARRAY_BUFFER, indexed_vertices[vboID+i].size() * sizeof(glm::vec3), &indexed_vertices[vboID+i][0], GL_STATIC_DRAW);
			//UVBUFFER
			glBindBuffer(GL_ARRAY_BUFFER, uvbuffer[vboID+i]);
			...
			//NORMALBUFFER
			glBindBuffer(GL_ARRAY_BUFFER, normalbuffer[vboID+i]);
			...
			//INDICEBUFFERS
			glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, elementbuffer[vboID+i]);
	...
	for(int ii = 0; ii < vboCount[initalVBO]; ii++)
	{
		if(dObj.drawDisable[vboID+ii] == 0)
		{
			...
			RotationMatrix = mat4_cast(gOrientation2[vboID+ii]);
			...
			*/
			...
			glBindTexture(GL_TEXTURE_2D, Texture[(vboID+ii)*3+0]);//dTexture
			glUniform1i(diffuseID[vboID+ii], 0);//Set "myTextureSampler" sampler to use Texture Unit 0

			glActiveTexture(GL_TEXTURE1);//Bind our texture in Texture Unit 1
			glBindTexture(GL_TEXTURE_2D, Texture[(vboID+ii)*3+1]);//sTexture
			glUniform1i(specularID[vboID+ii], 1);//Set "myTextureSampler" sampler to use Texture Unit 1

			glActiveTexture(GL_TEXTURE2);//Bind our texture in Texture Unit 2
			glBindTexture(GL_TEXTURE_2D, Texture[(vboID+ii)*3+2]);//eTexture
			glUniform1i(emissionID[vboID+ii], 2);//Set "myTextureSampler" sampler to use Texture Unit 2
			//}
			...
			glBindBuffer(GL_ARRAY_BUFFER, vertexbuffer[vboID+ii]);
			glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, (void*)0);
			//2nd attribute buffer : UVs
			glEnableVertexAttribArray(1);
			glBindBuffer(GL_ARRAY_BUFFER, uvbuffer[vboID+ii]);
			glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 0, (void*)0);
			//3rd attribute buffer : normals
			glEnableVertexAttribArray(2);
			glBindBuffer(GL_ARRAY_BUFFER, normalbuffer[vboID+ii]);
			...
			//Index buffer
			glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, elementbuffer[vboID+ii]);
