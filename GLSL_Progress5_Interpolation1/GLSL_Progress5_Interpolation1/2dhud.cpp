...
//extern GLuint diffuseID[90000], specularID[90000], emissionID[90000], Texture[90000], LightID[90000],
	//VertexArrayID[90000], vertexbuffer[90000], colorbuffer[90000], uvbuffer[90000], normalbuffer[90000], elementbuffer[90000];
	//programID, MatrixID, uniformModel, uniformView, uniformProjection;
...
void draw2dhud(int vboID, char*fName, float x, float y, float xwidth, float yheight)
{
	...
	{
		dObj.Texture[vboID*3+0] = loadPngImage(fName, imgwidth, imgheight, hasAlpha, &textureImage);//hudTexture
		//glGenBuffers(1, &hud2dVertexBufferID);
		//glGenBuffers(1, &hud2dUVBufferID);
		//TextureID[0] = glGetUniformLocation(programID, "myTexture");
		dObj.diffuseID[vboID] = glGetUniformLocation(dObj.programID, "myTexture");

		glGenVertexArrays(1, &dObj.VertexArrayID[vboID]);
		glBindVertexArray(dObj.VertexArrayID[vboID]);
		...
		//GLuint vertexbuffer;
		glGenBuffers(1, &dObj.vertexbuffer[vboID]);
		glBindBuffer(GL_ARRAY_BUFFER, dObj.vertexbuffer[vboID]);
		glBufferData(GL_ARRAY_BUFFER, sizeof(myVT), myVT, GL_STATIC_DRAW);
		//GLuint uvbuffer;
		glGenBuffers(1, &dObj.uvbuffer[vboID]);
		glBindBuffer(GL_ARRAY_BUFFER, dObj.uvbuffer[vboID]);
		glBufferData(GL_ARRAY_BUFFER, sizeof(myUV), myUV, GL_STATIC_DRAW);
		//GLuint elementbuffer;
		glGenBuffers(1, &dObj.elementbuffer[vboID]);
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, dObj.elementbuffer[vboID]);
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(myInd), myInd, GL_STATIC_DRAW);
	...
	{
		glCullFace(GL_BACK);
		glUseProgram(dObj.programID);//program2DshaderID
		...
		*/
		...
		dObj.projection = glm::ortho(0.0f, 800.f, 0.0f, 600.f, 0.1f, 100.0f);
		...
		dObj.TranslationMatrix = translate(mat4(), glm::vec3(0.0f, 0.0f, 0.0f));//ORIGIN AT BOTTOM LEFT
		...
		dObj.RotationMatrix = rotate(mat4(), 0.0f, glm::vec3(0.0f, 1.0f, 0.0f));
		dObj.ScalingMatrix = scale(mat4(), vec3(1.0f, 1.0f, 1.0f));//scalingX, scalingY, scalingZ));
		...
		dObj.view = LookAt(glm::vec3(0.0f, 0.0f, 1.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		...
		*/
		...
		glBindTexture(GL_TEXTURE_2D, dObj.Texture[vboID*3+0]);//hudTexture
...
void draw2dfont(int vboID, std::string text, float tempX, float tempY, float xwidth, float yheight)
{
	if(dObj.loadOnce[vboID] == 0)
	{
		...
		for(c = text.begin(); c != text.end(); c++)
		{
			...
			//GLuint vertexbuffer;
			glGenBuffers(1, &dObj.fontVertexBuffer[vboID][fontLoop[vboID]]);
			glBindBuffer(GL_ARRAY_BUFFER, dObj.fontVertexBuffer[vboID][fontLoop[vboID]]);
			glBufferData(GL_ARRAY_BUFFER, sizeof(myVT), myVT, GL_STATIC_DRAW);
			//GLuint elementbuffer;
			glGenBuffers(1, &dObj.fontElementBuffer[vboID][fontLoop[vboID]]);
			glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, dObj.fontElementBuffer[vboID][fontLoop[vboID]]);
	...
	{
		glCullFace(GL_BACK);
		glUseProgram(dObj.programFontID);//program2DshaderID
		int sw, sh;
		glfwGetWindowSize(window, &sw, &sh);
		double mxpos, mypos;
		glfwGetCursorPos(window, &mxpos, &mypos);
		//printf("Mouse X: %f Y: %f Screen W: %d H: %d\n", mxpos, mypos, sw, sh);
		////////////////////////////////////////////////////////////////////////////
		dObj.fontProjection = glm::ortho(0.0f, 800.f, 0.0f, 600.f, 0.1f, 100.0f);
		glUniformMatrix4fv(dObj.fontUniformProjection, 1, GL_FALSE, glm::value_ptr(dObj.fontProjection));

		dObj.fontTranslationMatrix = translate(mat4(), glm::vec3(0.0f, 0.0f, 0.0f));//ORIGIN AT BOTTOM LEFT
		dObj.fontRotationMatrix = rotate(mat4(), 0.0f, glm::vec3(0.0f, 1.0f, 0.0f));
		dObj.fontScalingMatrix = scale(mat4(), vec3(1.0f, 1.0f, 1.0f));//scalingX, scalingY, scalingZ));

		dObj.fontModelMatrix = dObj.fontTranslationMatrix * dObj.fontRotationMatrix * dObj.fontScalingMatrix;
		glUniformMatrix4fv(dObj.fontUniformModel, 1, GL_FALSE, glm::value_ptr(dObj.fontModelMatrix));

		dObj.fontView = LookAt(glm::vec3(0.0f, 0.0f, 1.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		//glm::mat4 view = LookAt(eye, eye+target, up);
		//glUniformMatrix4fv(uniformView2D[0], 1, GL_FALSE, glm::value_ptr(view2D));
		glUniformMatrix4fv(dObj.fontUniformView, 1, GL_FALSE, glm::value_ptr(dObj.fontView));

		dObj.fontMVP = dObj.fontProjection * dObj.fontView * dObj.fontModelMatrix;
		//glUniformMatrix4fv(uniformModel[0], 1, GL_FALSE, glm::value_ptr(model));
		//glUniformMatrix4fv(MatrixID2D, 1, GL_FALSE, &MVP2D[0][0]);//Send our transformation to the currently bound shader in the "MVP" uniform
		glUniformMatrix4fv(dObj.fontMatrixID, 1, GL_FALSE, &dObj.fontMVP[0][0]);//Send our transformation to the currently bound shader in the "MVP" uniform

		glUniform3f(glGetUniformLocation(dObj.programFontID, "textColor"), 1.0f, 1.0f, 0.0f);//YELLOW
		glUniform3f(glGetUniformLocation(dObj.programFontID, "textColor"), 0.0f, 0.0f, 0.0f);//BLACK
		///////////////////////////////////////////////////////////////////////////////////
		for(int i = 0; i < fontLoop[vboID]; i++)
		{
			...
			//1st attribute buffer : vertices
			glEnableVertexAttribArray(0);
			glBindBuffer(GL_ARRAY_BUFFER, dObj.fontVertexBuffer[vboID][i]);
			glVertexAttribPointer(0, 4, GL_FLOAT, GL_FALSE, 0, (void*)0);
			//INDICES
			glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, dObj.fontElementBuffer[vboID][i]);
