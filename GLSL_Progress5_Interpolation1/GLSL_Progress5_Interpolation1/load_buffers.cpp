...
float p1[90000], p2[90000], p3[90000], p4[90000], p5[90000], p6[90000];
float pRot1[90000], pRot2[90000], pRot3[90000], pRot4[90000], pRot5[90000], pRot6[90000];
float pScale1[90000], pScale2[90000], pScale3[90000], pScale4[90000], pScale5[90000], pScale6[90000];
vec3 pVec3Pos[90000], pVec3Rot[90000], pVec3Scale[90000];
...
//X TANGENT
vec2 tangentOutX[90000];//(myXOutTangent[i*2+0], myXOutTangent[i*2+1]);
vec2 tangentInX[90000];//(myXInTangent[i*2+0], myXInTangent[i*2+1]);
vec2 tangentOutXRot[90000];
vec2 tangentInXRot[90000];
vec2 tangentOutXScale[90000];
vec2 tangentInXScale[90000];
//Y TANGENT
vec2 tangentOutY[90000];//(myYOutTangent[i*2+0], myYOutTangent[i*2+1]);
vec2 tangentInY[90000];//(myYInTangent[i*2+0], myYInTangent[i*2+1]);
vec2 tangentOutYRot[90000];
vec2 tangentInYRot[90000];
vec2 tangentOutYScale[90000];
vec2 tangentInYScale[90000];
//Z TANGENT
vec2 tangentOutZ[90000];//(myZOutTangent[i*2+0], myZOutTangent[i*2+1]);
vec2 tangentInZ[90000];//(myZInTangent[i*2+0], myZInTangent[i*2+1]);
vec2 tangentOutZRot[90000];
vec2 tangentInZRot[90000];
vec2 tangentOutZScale[90000];
vec2 tangentInZScale[90000];
...
void drawVBO(int vboID, GLint myShader, char*modelfile, char*texturefile)
{
	...
	*/
	...
	if(dObj.loadOnce[vboID] == 0)
	{
		...
		{
			//printf("TESTXYZ\n");
			printf("Step: %d Control point: %d\n", steps, dObj.xInputCountControlPointID[vboID]);
			steps++;
		}
			...
			for(int t = 0; t < steps; t += 1)
			{
				...
				*/
				...
				if(i == 0)
				{
					...
					//X TANGENT
					tangentOutX[i] = vec2(dObj.myXOutTangent[i*2+0], dObj.myXOutTangent[i*2+1]);
					tangentInX[i] = vec2(dObj.myXInTangent[i*2+0], dObj.myXInTangent[i*2+1]);
					tangentOutXRot[i] = vec2(dObj.myXRotEulerOutTangent[i*2+0], dObj.myXRotEulerOutTangent[i*2+1]);
					tangentInXRot[i] = vec2(dObj.myXRotEulerInTangent[i*2+0], dObj.myXRotEulerInTangent[i*2+1]);
					tangentOutXScale[i] = vec2(dObj.myXScaleOutTangent[i*2+0], dObj.myXScaleOutTangent[i*2+1]);
					tangentInXScale[i] = vec2(dObj.myXScaleInTangent[i*2+0], dObj.myXScaleInTangent[i*2+1]);
					//Y TANGENT
					tangentOutY[i] = vec2(dObj.myYOutTangent[i*2+0], dObj.myYOutTangent[i*2+1]);
					tangentInY[i] = vec2(dObj.myYInTangent[i*2+0], dObj.myYInTangent[i*2+1]);
					tangentOutYRot[i] = vec2(dObj.myYRotEulerOutTangent[i*2+0], dObj.myYRotEulerOutTangent[i*2+1]);
					tangentInYRot[i] = vec2(dObj.myYRotEulerInTangent[i*2+0], dObj.myYRotEulerInTangent[i*2+1]);
					tangentOutYScale[i] = vec2(dObj.myYScaleOutTangent[i*2+0], dObj.myYScaleOutTangent[i*2+1]);
					tangentInYScale[i] = vec2(dObj.myYScaleInTangent[i*2+0], dObj.myYScaleInTangent[i*2+1]);
					//Z TANGENT
					tangentOutZ[i] = vec2(dObj.myZOutTangent[i*2+0], dObj.myZOutTangent[i*2+1]);
					tangentInZ[i] = vec2(dObj.myZInTangent[i*2+0], dObj.myZInTangent[i*2+1]);
					tangentOutZRot[i] = vec2(dObj.myZRotEulerOutTangent[i*2+0], dObj.myZRotEulerOutTangent[i*2+1]);
					tangentInZRot[i] = vec2(dObj.myZRotEulerInTangent[i*2+0], dObj.myZRotEulerInTangent[i*2+1]);
					tangentOutZScale[i] = vec2(dObj.myZScaleOutTangent[i*2+0], dObj.myZScaleOutTangent[i*2+1]);
					tangentInZScale[i] = vec2(dObj.myZScaleInTangent[i*2+0], dObj.myZScaleInTangent[i*2+1]);
				}
				else if(i == dObj.xInputCount)//LAST ONE
				{
					...
					p2[i] = dObj.myXOutput[i+0];//p2[i] = myXOutput[i*2+0];
					pRot1[i] = dObj.myXRotEulerOutput[i+0];
					pRot2[i] = dObj.myXRotEulerOutput[i+0];
					pScale1[i] = dObj.myXScaleOutput[i+0];
					pScale2[i] = dObj.myXScaleOutput[i+0];
					//Y POINTS
					p3[i] = dObj.myYOutput[i+0];//p3[i] = myYOutput[i*2-1];
					p4[i] = dObj.myYOutput[i+0];//p4[i] = myYOutput[i*2+0];
					pRot3[i] = dObj.myYRotEulerOutput[i+0];
					pRot4[i] = dObj.myYRotEulerOutput[i+0];
					pScale3[i] = dObj.myYScaleOutput[i+0];
					pScale4[i] = dObj.myYScaleOutput[i+0];
					//Z POINTS
					p5[i] = dObj.myZOutput[i+0];//p5[i] = myZOutput[i*2-1];
					p6[i] = dObj.myZOutput[i+0];//p6[i] = myZOutput[i*2+0];
					pRot5[i] = dObj.myZRotEulerOutput[i+0];
					pRot6[i] = dObj.myZRotEulerOutput[i+0];
					pScale5[i] = dObj.myZScaleOutput[i+0];
					pScale6[i] = dObj.myZScaleOutput[i+0];
					//X TANGENT
					tangentOutX[i] = vec2(dObj.myXOutTangent[i*2-1], dObj.myXOutTangent[i*2+0]);
					tangentInX[i] = vec2(dObj.myXInTangent[i*2-1], dObj.myXInTangent[i*2+0]);
					tangentOutXRot[i] = vec2(dObj.myXRotEulerOutTangent[i*2-1], dObj.myXRotEulerOutTangent[i*2+0]);
					tangentInXRot[i] = vec2(dObj.myXRotEulerInTangent[i*2-1], dObj.myXRotEulerInTangent[i*2+0]);
					tangentOutXScale[i] = vec2(dObj.myXScaleOutTangent[i*2-1], dObj.myXScaleOutTangent[i*2+0]);
					tangentInXScale[i] = vec2(dObj.myXScaleInTangent[i*2-1], dObj.myXScaleInTangent[i*2+0]);
					//Y TANGENT
					tangentOutY[i] = vec2(dObj.myYOutTangent[i*2-1], dObj.myYOutTangent[i*2+0]);
					tangentInY[i] = vec2(dObj.myYInTangent[i*2-1], dObj.myYInTangent[i*2+0]);
					tangentOutYRot[i] = vec2(dObj.myYRotEulerOutTangent[i*2-1], dObj.myYRotEulerOutTangent[i*2+0]);
					tangentInYRot[i] = vec2(dObj.myYRotEulerInTangent[i*2-1], dObj.myYRotEulerInTangent[i*2+0]);
					tangentOutYScale[i] = vec2(dObj.myYScaleOutTangent[i*2-1], dObj.myYScaleOutTangent[i*2+0]);
					tangentInYScale[i] = vec2(dObj.myYScaleInTangent[i*2-1], dObj.myYScaleInTangent[i*2+0]);
					//Z TANGENT
					tangentOutZ[i] = vec2(dObj.myZOutTangent[i*2-1], dObj.myZOutTangent[i*2+0]);
					tangentInZ[i] = vec2(dObj.myZInTangent[i*2-1], dObj.myZInTangent[i*2+0]);
					tangentOutZRot[i] = vec2(dObj.myZRotEulerOutTangent[i*2-1], dObj.myZRotEulerOutTangent[i*2+0]);
					tangentInZRot[i] = vec2(dObj.myZRotEulerInTangent[i*2-1], dObj.myZRotEulerInTangent[i*2+0]);
					tangentOutZScale[i] = vec2(dObj.myZScaleOutTangent[i*2-1], dObj.myZScaleOutTangent[i*2+0]);
					tangentInZScale[i] = vec2(dObj.myZScaleInTangent[i*2-1], dObj.myZScaleInTangent[i*2+0]);
				}
				else
				{
					//X POINTS
				...
				pX = vec2(h1*p1[i]) +                    // multiply and sum all funtions
					...
					vec2(h3*tangentOutX[i]) +                    // point along the curve.
					vec2(h4*tangentInX[i]);
				pVec3Pos[i].x = pX.x;//p1[i] = pX.x;

				pY = vec2(h1*p3[i]) +                    // multiply and sum all funtions
					vec2(h2*p4[i]) +                    // together to build the interpolated
					vec2(h3*tangentOutY[i]) +                    // point along the curve.
					vec2(h4*tangentInY[i]);
				pVec3Pos[i].y = pY.x;//p3[i] = pY.x;

				pZ = vec2(h1*p5[i]) +                    // multiply and sum all funtions
					vec2(h2*p6[i]) +                    // together to build the interpolated
					vec2(h3*tangentOutZ[i]) +                    // point along the curve.
					vec2(h4*tangentInZ[i]);
				pVec3Pos[i].z = pZ.x;//p5[i] = pZ.x;
				//printf("pVec3 X: %f Y: %f Z: %f\n", pVec3Pos.x, pVec3Pos.y, pVec3Pos.z);

				/////////////////////////////////////////////////////////////////////////////

				pRotX = vec2(h1*pRot1[i]) +                    // multiply and sum all funtions
					vec2(h2*pRot2[i]) +                    // together to build the interpolated
					vec2(h3*tangentOutXRot[i]) +                    // point along the curve.
					vec2(h4*tangentInXRot[i]);
				pVec3Rot[i].x = pRotX.x;//pRot1[i] = pRotX.x;

				pRotY = vec2(h1*pRot3[i]) +                    // multiply and sum all funtions
					vec2(h2*pRot4[i]) +                    // together to build the interpolated
					vec2(h3*tangentOutYRot[i]) +                    // point along the curve.
					vec2(h4*tangentInYRot[i]);
				pVec3Rot[i].y = pRotY.x;//pRot3[i] = pRotY.x;

				pRotZ = vec2(h1*pRot5[i]) +                    // multiply and sum all funtions
					vec2(h2*pRot6[i]) +                    // together to build the interpolated
					vec2(h3*tangentOutZRot[i]) +                    // point along the curve.
					vec2(h4*tangentInZRot[i]);
				pVec3Rot[i].z = pRotZ.x;//pRot5[i] = pRotZ.x;
				
				/////////////////////////////////////////////////////////////////////////////

				pScaleX = vec2(h1*pScale1[i]) +                    // multiply and sum all funtions
					vec2(h2*pScale2[i]) +                    // together to build the interpolated
					vec2(h3*tangentOutXScale[i]) +                    // point along the curve.
					vec2(h4*tangentInXScale[i]);
				pVec3Scale[i].x = pScaleX.x;//pScale1[i] = pScaleX.x;

				pScaleY = vec2(h1*pScale3[i]) +                    // multiply and sum all funtions
					vec2(h2*pScale4[i]) +                    // together to build the interpolated
					vec2(h3*tangentOutYScale[i]) +                    // point along the curve.
					vec2(h4*tangentInYScale[i]);
				pVec3Scale[i].y = pScaleY.x;//pScale3[i] = pScaleY.x;

				pScaleZ = vec2(h1*pScale5[i]) +                    // multiply and sum all funtions
					vec2(h2*pScale6[i]) +                    // together to build the interpolated
					vec2(h3*tangentOutZScale[i]) +                    // point along the curve.
					vec2(h4*tangentInZScale[i]);
				pVec3Scale[i].z = pScaleZ.x;//pScale5[i] = pScaleZ.x;
				//printf("pScale: %f\n", pScale1[i]);
	...
	for(int ii = 0; ii < vboCount[initalVBO]; ii++)
	{
		if(dObj.drawDisable[vboID+ii] == 0)
		{
			...
			if(testTime < 1.0f)
			{
				if(dObj.myXInterpolation[playLoop] == 2)//0 AS CUBIC, 1 AS LINEAR, 2 AS STEP (CONSTANT)
				{
					if(testTime < 1.0f)
					{
						//LOC
						daeLocX = ((1.0f - 0) * p1[playLoop] + (0 * p2[playLoop]));
						daeLocY = ((1.0f - 0) * p3[playLoop] + (0 * p4[playLoop]));
						daeLocZ = ((1.0f - 0) * p5[playLoop] + (0 * p6[playLoop]));
						//printf("t: %f daeLocX: %f\n", testTime, daeLocX);
						//ROT
						daeRotEulerX = ((1.0f - 0) * pRot1[playLoop] + (0 * pRot2[playLoop]));
						daeRotEulerY = ((1.0f - 0) * pRot3[playLoop] + (0 * pRot4[playLoop]));
						daeRotEulerZ = ((1.0f - 0) * pRot5[playLoop] + (0 * pRot6[playLoop]));
						//printf("daeRot X: %f Y: %f Z: %f\n", daeRotEulerX, daeRotEulerY, daeRotEulerZ);
						//SCALE
						daeScaleX = ((1.0f - 0) * pScale1[playLoop] + (0 * pScale2[playLoop]));
						daeScaleY = ((1.0f - 0) * pScale3[playLoop] + (0 * pScale4[playLoop]));
						daeScaleZ = ((1.0f - 0) * pScale5[playLoop] + (0 * pScale6[playLoop]));
				...
				else if(dObj.myXInterpolation[playLoop] == 1)//LINEAR
				{
					//LOC
					daeLocX = ((1.0f - testTime) * p1[playLoop] + (testTime * p2[playLoop]));
					daeLocY = ((1.0f - testTime) * p3[playLoop] + (testTime * p4[playLoop]));
				...
				//else if(dObj.myXInterpolation[playLoop] == 0)
				//CUBIC SPLINE
				else
				{
					//HERMITE CURVES
					...
					previousTangent = deltaTime * vec3(tangentOutX[playLoop].x, tangentOutY[playLoop].x, tangentOutZ[playLoop].x);
					nextTangent = deltaTime * vec3(tangentInX[playLoop].x, tangentInY[playLoop].x, tangentInZ[playLoop].x);
					...
					//tempVec3Pos = vec3(p1[playLoop], p3[playLoop], p5[playLoop]) * ((1.0f + 2.0f * testTime) * ((1.0f - testTime) * (1.0f - testTime))) +
						//vec3(tangentOutX[playLoop].x, tangentOutY[playLoop].x, tangentOutZ[playLoop].x) * (testTime * ((1.0f - testTime) * (1.0f - testTime))) +
						//vec3(p2[playLoop], p4[playLoop], p6[playLoop]) * ((testTime * testTime) * (3.0f - 2.0f * testTime)) +
						//vec3(tangentInX[playLoop].x, tangentInY[playLoop].x, tangentInZ[playLoop].x) * ((testTime * testTime) * (testTime - 1.0f));

					tempVec3Pos = vec3(p1[playLoop], p3[playLoop], p5[playLoop]) * ((1.0f + 2.0f * testTime) * ((1.0f - testTime) * (1.0f - testTime))) +
						previousTangent * (testTime * ((1.0f - testTime) * (1.0f - testTime))) +
						vec3(p2[playLoop], p4[playLoop], p6[playLoop]) * ((testTime * testTime) * (3.0f - 2.0f * testTime)) +
						nextTangent * ((testTime * testTime) * (testTime - 1.0f));
					...
					//ROT
					previousTangent = deltaTime * vec3(tangentOutXRot[playLoop].x, tangentOutYRot[playLoop].x, tangentOutZRot[playLoop].x);
					nextTangent = deltaTime * vec3(tangentInXRot[playLoop].x, tangentInYRot[playLoop].x, tangentInZRot[playLoop].x);

					tempVec3Rot = vec3(pRot1[playLoop], pRot3[playLoop], pRot5[playLoop]) * ((1.0f + 2.0f * testTime) * ((1.0f - testTime) * (1.0f - testTime))) +
						previousTangent * (testTime * ((1.0f - testTime) * (1.0f - testTime))) +
						vec3(pRot2[playLoop], pRot4[playLoop], pRot6[playLoop]) * ((testTime * testTime) * (3.0f - 2.0f * testTime)) +
						nextTangent * ((testTime * testTime) * (testTime - 1.0f));
					...
					tempVec3Scale = vec3(pScale1[playLoop], pScale3[playLoop], pScale5[playLoop]) * ((1.0f + 2.0f * testTime) * ((1.0f - testTime) * (1.0f - testTime))) +
						previousTangent * (testTime * ((1.0f - testTime) * (1.0f - testTime))) +
						vec3(pScale2[playLoop], pScale4[playLoop], pScale6[playLoop]) * ((testTime * testTime) * (3.0f - 2.0f * testTime)) +
						nextTangent * ((testTime * testTime) * (testTime - 1.0f));

					daeScaleX = tempVec3Scale.x;
					daeScaleY = tempVec3Scale.y;
					daeScaleZ = tempVec3Scale.z;
					//printf("playLoop: %d daeScale X: %f Y: %f Z: %f\n", playLoop, daeScaleX, daeScaleY, daeScaleZ);//_getch();
				}
				testTime += 0.1f;//testTime += 0.001f;
			}
			else
			{
				testTime = 0.0f;
				if(playLoop > playCount[vboID] - 1)
				{
					playLoop = 0;
				}
				else
			...
			*/
			...
			if(dObj.xScaleInputFoundArray[vboID] == 1)
			{
			...
			{
				dObj.ScalingMatrix = scale(mat4(), vec3(scalingX, scalingY, scalingZ));//1.0f, 1.0f, 1.0f));
				//printf("scaleTest2: %d\n", vboID);
			}
