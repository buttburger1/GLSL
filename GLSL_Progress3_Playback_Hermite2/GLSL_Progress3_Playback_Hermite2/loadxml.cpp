	...
	for(cur_node = a_node; cur_node; cur_node = cur_node->next)
	{
		if(cur_node->type == XML_ELEMENT_NODE)
		{
			...
			while(attribute)
			{
				...
				if(limitOne < (meshCount * 3) && strcmp((char*)attribute->children->content, meshAppendPositions) == 0)
				{
				...
				if(limitOne < (meshCount * 3) && strcmp((char*)attribute->children->content, meshAppendNormals) == 0)
				{
					if(meshSkip <= limitOne)
					{
						normalsfound = 1;
				...
				if(limitOne < (meshCount * 3) && strcmp((char*)attribute->children->content, meshAppendMap0) == 0)
				{
					if(meshSkip <= limitOne)
					{
						uvmapfound = 1;
