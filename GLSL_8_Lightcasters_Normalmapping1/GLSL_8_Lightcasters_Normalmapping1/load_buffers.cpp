...
void drawVBO(int vboID, GLint myShader, char*modelfile, char*texturefile, int vboTRS)
{
	...
	if(dObj.loadOnce[vboID] == 0)
	{
		...
		//NEED TO SPLIT FOR MULTIPLE OBJECTS IN 1 FILE
		...
		for(int i = 0; i <= dObj.meshVBO; i++)
		{
			printf("i: %d meshVBO: %d vboID: %d \n", i, dObj.meshVBO, vboID);
			dObj.currentMesh = 0;
		...
		for(int i = 0; i < dObj.meshCount; i++)
		{
			...
			vboCount[vboID+i] = dObj.meshCount;
			//printf("vboCount: %d\n", vboCount[vboID]);
			printf("vboCount: %d\n", vboCount[vboID+i]);
			...
			dObj.meshLoad[vboID+i] = dObj.testMeshCount;
		}
		/*
		vboCount[vboID] = dObj.meshCount;
		vboCount[vboID] = dObj.meshCount;
	...
	*/
	...
	//for(int ii = 0; ii < vboCount[initalVBO+ii]; ii++)
	//for(int ii = 0; ii < dObj.meshLoad[vboID]; ii++)
	{
		//printf("II loop: %d\n", ii);
		//printf("ii: %d vboCount: %d initialVBO: %d\n", ii, vboCount[initalVBO+ii], initalVBO);
		//printf("test: %d\n", dObj.vboNumber);
		...
		if(dObj.drawDisable[vboID+ii] == 0)
		{
			...
			*/
			...
			if(dObj.vboNumber < vboOrderCount)
			{
				...
				if(vboOrderArray[dObj.vboNumber] == vboID)
				{
					//printf("2 ");
					...
					if(testTime[dObj.vboNumber] < 1.0f && trsSwapArray[dObj.vboNumber] > 0)
					{
					...
					if(trsSwapArray[dObj.vboNumber] == 0)
					{
						...
						if(vboTRS == 1)
						{
							...
							glm::vec3 lightPos(modelX, modelY, modelZ);//OUTSIDE_ORBIT
							dObj.TranslationMatrix = glm::mat4(1.0f);
			...
			*/
			...
			//DIRECTIONAL LIGHT
			char dirLight[255];
			strcpy(dirLight, "dirLight.direction");
			...
			//POINT LIGHT
			glUniform3f(glGetUniformLocation(myShader, "pointLight.position"), 5.0f, 5.0f, 0.0f);//modelX, modelY, modelZ);
			glUniform3f(glGetUniformLocation(myShader, "pointLight.ambient"), 0.5f, 0.5f, 0.5f);//1.0f, 1.0f, 1.0f);
			glUniform3f(glGetUniformLocation(myShader, "pointLight.diffuse"), 0.6f, 0.6f, 0.6f);//1.0f, 1.0f, 1.0f);
			glUniform3f(glGetUniformLocation(myShader, "pointLight.specular"), 0.7f, 0.7f, 0.7f);//1.0f, 1.0f, 1.0f);
			glUniform1f(glGetUniformLocation(myShader, "pointLight.constant"), 1.0f);
			glUniform1f(glGetUniformLocation(myShader, "pointLight.linear"), 0.09f);
			glUniform1f(glGetUniformLocation(myShader, "pointLight.quadratic"), 0.032f);
			
			//SPOT LIGHT
			glUniform3f(glGetUniformLocation(myShader, "spotLight.position"), eye.x, eye.y, eye.z);//modelX, modelY, modelZ);
			glUniform3f(glGetUniformLocation(myShader, "spotLight.direction"), dObj.target.x, dObj.target.y, dObj.target.z);
			glUniform3f(glGetUniformLocation(myShader, "spotLight.ambient"), 0.1f, 0.1f, 0.1f);
			glUniform3f(glGetUniformLocation(myShader, "spotLight.diffuse"), 1.0f, 1.0f, 1.0f);
			glUniform3f(glGetUniformLocation(myShader, "spotLight.specular"), 1.0f, 1.0f, 1.0f);
			glUniform1f(glGetUniformLocation(myShader, "spotLight.constant"), 1.0f);
			glUniform1f(glGetUniformLocation(myShader, "spotLight.linear"), 0.09f);
			glUniform1f(glGetUniformLocation(myShader, "spotLight.quadratic"), 0.032f);
			glUniform1f(glGetUniformLocation(myShader, "spotLight.cutOff"), glm::cos(glm::radians(12.5f)));
			glUniform1f(glGetUniformLocation(myShader, "spotLight.outerCutOff"), glm::cos(glm::radians(15.0f)));
			...
			//TEST FOR EMISSION
			if(dObj.materialVBOStoredUV[vboID+ii] == 1)glUniform1i(glGetUniformLocation(myShader, "shaderFlow"), 1);
			else if(dObj.materialVBOStoredColor[vboID+ii] == 1)glUniform1i(glGetUniformLocation(myShader, "shaderFlow"), 2);
