...
extern glm::vec3 eye;
extern glm::vec3 target, right, up;
...
void drawPoints(int vboID)
{
	...
	if(dObj.loadOnce[vboID] == 0)
	{
		...
		//GLuint vertexbuffer;
		glGenBuffers(1, &shapesVertexBuffer[vboID]);
		glBindBuffer(GL_ARRAY_BUFFER, shapesVertexBuffer[vboID]);
		glBufferData(GL_ARRAY_BUFFER, sizeof(pointVertex), pointVertex, GL_STATIC_DRAW);
		//GLuint uvbuffer;
		glGenBuffers(1, &shapesColorBuffer[vboID]);
		glBindBuffer(GL_ARRAY_BUFFER, shapesColorBuffer[vboID]);
	...
	{
		glCullFace(GL_BACK);
		glUseProgram(programShapesShaderID);
		//2D
		//shapesProjection = glm::ortho(0.0f, 800.f, 0.0f, 600.f, 0.1f, 100.0f);
		//3D
		shapesProjection = glm::perspective(glm::radians(60.0f), (float)SCREEN_WIDTH/(float)SCREEN_HEIGHT, 0.1f, 1000.0f);
		glUniformMatrix4fv(shapesUniformProjection, 1, GL_FALSE, glm::value_ptr(shapesProjection));
		...
		shapesRotationMatrix = mat4_cast(gOrientation2[vboID]);
		...
		//3D
		shapesTranslationMatrix = translate(mat4(), glm::vec3(1.0f, 1.0f, 1.0f));//modelX, modelY, modelZ));

		//shapesScalingMatrix = scale(mat4(), vec3(scalingX, scalingY, scalingZ));//1.0f, 1.0f, 1.0f));
		shapesScalingMatrix = scale(mat4(), vec3(1.0f, 1.0f, 1.0f));
		shapesModelMatrix = shapesTranslationMatrix * shapesRotationMatrix * shapesScalingMatrix;
		...
		glUniformMatrix4fv(shapesUniformView, 1, GL_FALSE, glm::value_ptr(view));
		//glm::mat4 model = glm::translate(glm::mat4(1.0f), glm::vec3(modelx, modely, modelz));
		glUniformMatrix4fv(shapesUniformModel, 1, GL_FALSE, glm::value_ptr(shapesModelMatrix));
		shapesMVP = shapesProjection * view * shapesModelMatrix;
		glUniformMatrix4fv(shapesMatrixID, 1, GL_FALSE, &shapesMVP[0][0]);//Send our transformation to the currently bound shader in the "MVP" uniform
